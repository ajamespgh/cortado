...
const projectRoot = (configPath) => configPath.includes("/") ? configPath.slice(0, configPath.lastIndexOf("/")) : ".";
...
function ProjectCoverage({ projects = [], activeProject, onProjectChange }) {
  if (!projects.length) return null;
  return <section className="project-coverage"><div className="section-title">TypeScript projects</div><button className={activeProject === "all" ? "project-filter active-project" : "project-filter"} onClick={() => onProjectChange("all")}>All projects <small>{projects.reduce((total, item) => total + item.files.length, 0)} files</small></button>{projects.map((item) => <button key={item.id} className={activeProject === item.id ? "project-filter active-project" : "project-filter"} onClick={() => onProjectChange(item.id)} title={`Configuration: ${item.configPath}`}><span>{projectRoot(item.configPath)}</span><small className={item.status === "ready" ? "project-ready" : "project-error"}>{item.status} · {item.files.length} files{item.diagnostics.length ? ` · ${item.diagnostics.length} diagnostics` : ""}</small></button>)}</section>;
}

function Explorer({ workspace, projects, activeProject, onProjectChange, selected, onSelect, collapsed, onToggle }) {
...
function App() {
  ...
  useEffect(() => {
    ...
    const focusFiles = activeProject === "all" ? null : new Set(project?.projects.find((item) => item.id === activeProject)?.files || []);
    const visiblePaths = !focusFiles ? null : new Set([".", ...[...focusFiles].flatMap((file) => { const parts = file.split("/"); return parts.slice(0, -1).map((_, index) => parts.slice(0, index + 1).join("/")); })]);
    const visible = visiblePaths ? all.filter((item) => item.kind !== "file" ? visiblePaths.has(item.path) : focusFiles.has(item.path)) : all;
    const positions = conceptMapLayout(visible);
    const nextNodes = visible.map((item) => ({ id: item.id, position: positions.get(item.id), type: "compact", data: { kind: item.kind === "file" && /\.tsx$/i.test(item.path) ? "tsx" : item.kind === "file" && /\.ts$/i.test(item.path) ? "typescript" : item.kind === "file" && /\.jsx$/i.test(item.path) ? "jsx" : item.kind === "file" && /\.js$/i.test(item.path) ? "javascript" : item.kind === "file" && /\.json$/i.test(item.path) ? "json" : item.kind === "file" && /\.(css|scss|less)$/i.test(item.path) ? "css" : item.kind === "file" && /\.md$/i.test(item.path) ? "markdown" : item.kind, title: item.kind === "root" ? workspace.root : item.path }, style: { background: item.kind === "root" ? "#075985" : item.kind === "folder" ? "#422006" : "#172554", color: "#e5e7eb", border: item.kind === "root" ? "2px solid #fbbf24" : "1px solid #60a5fa", borderRadius: "50%", padding: 3, width: 42, height: 42 } }));
    const handlePair = (source, target) => directionalHandles(positions.get(source) || { x: 0, y: 0 }, positions.get(target) || { x: 0, y: 0 });
    const visibleIds = new Set(visible.map((item) => item.id));
    const hierarchy = visible.slice(1).map((item) => { const parent = item.path.includes("/") ? item.path.slice(0, item.path.lastIndexOf("/")) : "."; const source = `workspace:${parent}`; return { id: `contains:${parent}:${item.path}`, source, target: item.id, type: "default", relationType: "containment", ...handlePair(source, item.id) }; });
    const imports = (project?.edges || []).filter((item) => visibleIds.has(`workspace:${item.from}`) && visibleIds.has(`workspace:${item.to}`)).map((item, index) => { const source = `workspace:${item.from}`; const target = `workspace:${item.to}`; return { id: `import:${index}`, source, target, type: "default", relationType: "import", ...handlePair(source, target) }; });
